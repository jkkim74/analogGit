(function ($) {
    ...
    $(document).ready(function () {
        ...
        if ($.fn.niceScroll) {
			...
			$(".bookmark-area").getNiceScroll().resize();
            if ($('#sidebar').hasClass('hide-left-bar')) {
                $(".leftside-navigation").getNiceScroll().hide();
				$(".bookmark-area").getNiceScroll().hide();
            }
		...
		$('.edit-dashboard').click(function(){
			...
			$('.content-wrap').toggleClass('edit-mode');
		});
		$(".variation").variationWidget();
	});
	...
	$.fn.extend({
		...
		//즐겨찾기
		lnbBookMark: function(){
			var _this = $(this);
			if (!_this.get(0)){return false};
			var bookmark = _this.find(".bookmark-area");
			$.each($(this),function(){
		...
		// 데이터 위젯
		variationWidget:function(){
			var _this = $(this);
			if (!_this.get(0)){return false};
			$(this).each(function(){
				var listLength = $(this).find('tr').length;
				if (listLength > 4){
					$(this).parent().prev('.variation-thead').find('tr th:last-child').css('width','48px');
				}else {
					$(this).parent().prev('.variation-thead').css('width','100%');
				}
			});
		}
	});
...
$(window).load(function(){
	$(".dashboard-lnb").dashboardLnb();
	$(".bookmark-area").lnbBookMark();
	$(".pivot-board01 .pvtRendererArea table.pvtTable").pivotBoard();
	$(".bookmark-modal").bookmarkModal();
	// pivot total 관련 엘리먼트 제거
	$(".pvtTotal, .pvtTotalLabel, .pvtGrandTotal").remove();
});
