function oneidJoinPcCtrl2($scope, DATE_TYPE_DAY, reportSvc, $timeout) {
    $scope.init = function () {
    ...
    function drawFunnel() {
        ...
        var url = '/oneid/funnel/chart?inoutFlg=1&dateType=' + $scope.searchDateType + '&startDate=' + $scope.searchStartDate + '&endDate=' + $scope.searchEndDate;

        reportSvc.getReportApi(url).then(function (result) {
            if (result && result.S1) {
                $scope.$safeApply(function () {
                    $scope.funnelId = 'oneidFunnel2';
