function bpmEmailCtrl($scope, adminSvc, userSvc, ngDialog, blockUI) {
    ...
    $scope.init = function () {
        ...
        $scope.ocbDate = DateHelper.getPreviousDate(2, 'YYYYMMDD');//"20140715";
        $scope.thisDate = $scope.previousDate.substring(4, 6) + "." + $scope.previousDate.substring(6);
        $scope.thisOcbDate = $scope.ocbDate.substring(4, 6) + "." + $scope.ocbDate.substring(6);
        $scope.thisMonth = $scope.previousDate.substring(4, 6);
        $scope.previousCommaDate = DateHelper.stringToYmdStr($scope.previousDate, '.');
        $scope.lastMonth = DateHelper.getPreviousMonth(1, 'YYYYMM').substring(4);//DateHelper.dateToParamString(DateHelper.getLastMonth(new Date(), 1)).substring(4, 6);
        $scope.monthBeforeLast = DateHelper.getPreviousMonth(2, 'YYYYMM').substring(4);
        callBpmEmailResult();
    };
    ...
    function callBpmEmailResult() {
        ...
        var emailUrl = '/admin/boss/emailUsers?sndObjId=1';
    ...
    function getMailReportData() {
        adminSvc.getMailData($scope.previousDate).then(function (result) {
            if (result) {
                $scope.mailReports = (result.reports) ? result.reports : '';
                ...
                if ($scope.addable) {
                    ...
                    adminSvc.createMailUser($scope.createMailGridData).then(function (createResult) {
                        if (createResult.code === 200) {
                            alert("메일링 리스트 추가 처리가 정상적으로 이루어졌습니다.");
                        } else {
                            alert("메일링 리스트 추가 처리가 실패했습니다.");
                        }
                    });
                }
                $scope.preViewFlag = false;
    ...
    $scope.commentSave = function () {
        ...
        adminSvc.createMailComments($scope.form).then(function (result) {
            if (result.code !== 200) {
                alert('저장과정에 오류가 발생했습니다. 관리자에게 문의해 주세요.');
                return false;
            } else {
    ...
    $scope.preView = function () {
        ...
        adminSvc.getMailContents($scope.getMailGridData).then(function (result) {
            $scope.mailContents = result;
            ngDialog.open({
                template: $scope.mailContents.emailCtxtCtt,
                plain: true,
                scope: $scope,
                controller: 'bpmEmailCtrl',
    ...
    $scope.sendMail = function () {
        ...
        adminSvc.sendMails($scope.mailDatas).then(function (result) {
            blockUI.stop();
            if (result.code === 200) {
                alert("메일이 정상적으로 발송되었습니다.");
            } else {
                alert("일부 혹은 전체 메일 발송이 실패했습니다.");
            }
        });
    };
}
