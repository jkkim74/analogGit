/**
 * Created by cookatrice on 2015. 8. 6..
 */

function sankeyCtrl($scope, $log, apiSvc,reportSvc) {

    $scope.init = function () {
        var defaultCal = reportSvc.defaultCal();
        $scope.sankeyCahrtId = '#sankeyChart';
        $scope.searchConfig = {
        ...
        $scope.searchStartDate = $scope.searchConfig.startDate;
        $scope.searchEndDate = $scope.searchConfig.endDate;
        $scope.searchSite = $scope.searchConfig.site;
        $scope.searchService = $scope.searchConfig.service;
        $scope.searchType = $scope.searchConfig.type;
    ...
    $scope.search = function (result) {
        ...
        $scope.searchSite = result.searchSite;
        //$scope.searchService = result.searchService;
        $scope.searchType = result.searchType;
        $scope.firstDepthName = result.firstDepthName;

        loadSankeyChartDataSet();
    };

    function clearSankeyCahrt(){
         if (d3.select($scope.sankeyCahrtId).selectAll('svg')){
            d3.select($scope.sankeyCahrtId).selectAll('svg').remove();
        }
    }

    //for test... :)
    $scope.moveSankeyStep = function(toMove){
        moveSankeyStep(toMove);
    };
    ...
    function loadSankeyChartDataSet() {
        clearSankeyCahrt();
        $scope.receiveRawData = null;
        $scope.sankeyChartMsg = '조회중입니다....';

        var httpConfig = {
            url: '/sankey/getDataSet',
            params: {
                startDate: $scope.searchStartDate,
                endDate: $scope.searchEndDate,
                site: $scope.searchSite,
                service: $scope.searchService,
                type : $scope.searchType,
                maxDepth : $scope.searchConfig.maxDepth,
                minValue : $scope.searchConfig.minValue
            }
        };
        apiSvc.voyagerHttpSvc(httpConfig).then(function (result) {
            ...
            if (receiveData && receiveData.length != 0) {
                ...
                $scope.sankeyChartMsg = '조회결과가 없습니다.';
           }
        });
    }
    ...
    function extractCurDepthSankeyData(sankeyData, depth) {
        ...
        /** extract nodes */
        _.each(oldNodes, function(element){
        ...
        /** extract links */
        _.each(oldLinks, function (element) {
            //exist link
            if (element.depth <= depth) {
        ...
        if(depth > 1) {
            ...
            _.each(nodeInOutStatus, function (element) {
                var diffInOut = element.in - element.out;
                var calDepth = (newNodes[element.node].depth < depth);
                if(diffInOut > 0 && calDepth){
                    newLinks.push({source:element.node, target:endNodeIndex, value:element.in-element.out, depth:depth});
                }
            });
        }
    ...
    function drawSankeyChart(refineSankeyData) {
        ...
        $($scope.sankeyCahrtId).css("width", width + margin.left + margin.right).css("height", height + margin.top + margin.bottom);
        ...
        if (sankeyDataSet) {
            ...
            depthInfo.append("text")
                ...
                .text(function (d) {
                    if(d.depth < 1){
                       return $scope.firstDepthName?$scope.firstDepthName:'방문페이지';
                    }
                    return d.depth + 'Step';
                })
                .append('svg:tspan')
                ...
                .attr('dy', 20)
                .text(function (d) {
                    var returnString = '세션 : ' + d.in + ', 경유 : ' + d.out;
                    if(d.depth > 0){
                        returnString += ', 이탈 : ' + (d.in - d.out);
                    }
                    return returnString;
                })
                .attr("fill", "black");
            ...
            var tip = d3.tip()
                ...
                .html(function (d) {
                    ...
                    return "<strong>* Page :</strong> <span style='color:lightskyblue'>" + d.name + "</span>"
                        + "<br/><br/><strong>* 경유트래픽 :</strong> <span style='color:yellow'>" + outVal + " ("+Math.round((outVal/inVal*100)*100)/100 +"%)" + "</span>"
                        + "<br/><br/><strong>* 이탈수 :</strong> <span style='color:pink'>"   + outPage+ " ("+Math.round((outPage/inVal*100)*100)/100 +"%)" + "</span>";
                });
