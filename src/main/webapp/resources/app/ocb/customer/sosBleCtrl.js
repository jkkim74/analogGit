...
function sosBleCtrl($scope, DATE_TYPE_DAY, reportSvc) {
    ...
    function gridSosBle(datas) {
        var prevCell1 = { cellId: undefined, value: undefined };
        var prevPrevCell1 = "";

        //grid
        $scope.sosBleForGridConfig = {
            data: datas,
            datatype: "local",
            height: "100%",
            shrinkToFit: false,
        ...
        angular.element("#sosBleForGrid").html($table);
        angular.element("#" + $scope.tableId).jqGrid("GridUnload");
        angular.element("#" + $scope.tableId).jqGrid($scope.sosBleForGridConfig);
    }
