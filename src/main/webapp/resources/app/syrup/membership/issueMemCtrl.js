function issueMemCtrl($scope, DATE_TYPE_DAY, reportSvc) {
    $scope.init = function () {
        ...
        $scope.drawMembershipIssueMem();
    };

    $scope.drawMembershipIssueMem = function () {
        $scope.pageState = {
            'loading': true,
            'emptyData': true
        };
        var url = '/syrup/membership/cardissuemem/grid?dateType=' + $scope.searchDateType + '&startDate='
            + $scope.searchStartDate + '&endDate=' + $scope.searchEndDate + '&sidx=stdDt&sord=asc';

        $scope.tableId = 'mebershipIssueMemTableForGrid';
        reportSvc.getReportApi(url).then(function(result) {
            var dataLength = result.rows.length;
            if (dataLength != 0) {
                gridMembershipIssueMem(result.rows);
            } else {
    ...
    function gridMembershipIssueMem(datas) {
        $scope.issueMemForGridConfig = {
            data : datas,
            datatype: "local",
            height: "100%",
            shrinkToFit: true,
        ...
        angular.element("#mebershipIssueMemForGrid").html($table);
        angular.element("#" + $scope.tableId).jqGrid("GridUnload");
        angular.element("#" + $scope.tableId).jqGrid($scope.issueMemForGridConfig);
    }

    $scope.$on('$destroy', function() {
        $('#mebershipIssueMemForGrid').empty();
    });
     ...
     */
    $scope.excelConfig = {
        downloadUrl : '/syrup/membership/downloadExcelForMebershipIssueMem',
        downloadType : 'POI',
        pivotFlag : 'N',
        tableId: 'mebershipIssueMemForGrid',
        xlsName: 'membership-issue-mem.xls'
    };
}
