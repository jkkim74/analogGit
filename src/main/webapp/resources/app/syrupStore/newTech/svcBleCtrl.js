function svcBleCtrl($scope, reportSvc) {
    $scope.init = function () {
        ...
        $scope.searchConfig = {
            ...
            searchItems: [
                ...
                {code: '2', displayName:'가맹점명'},
                {code: '3', displayName:'MID'},
...
//            ]
    ...
    $scope.search = function (result) {
        ...
        $scope.group1Headers = [];
        $scope.group2Headers = [];
        if ($scope.searchDateType == 'sum' || $scope.searchDateType == 'day') {
           if (!$scope.searchString) {
               if (DateHelper.diff($scope.searchStartDate, $scope.searchEndDate) > 3) {
                   alert('조회 데이터가 많아서 5일 이상은 조회할 수 없습니다.')
                   return;
               }
           } else {
               if ($scope.searchCode == 1) {
                   if (DateHelper.diff($scope.searchStartDate, $scope.searchEndDate) > 28) {
                       alert('Campaign ID 항목검색은 조회 데이터가 많아서 30일 이상은 조회할 수 없습니다.')
                       return;
                   }
               } else {
                   if (DateHelper.diff($scope.searchStartDate, $scope.searchEndDate) > 88) {
                       alert('Campaign ID 제외한 항목검색은 조회 데이터가 많아서 90일 이상은 조회할 수 없습니다.')
                       return;
                   }
               }
           }
    ...
    function getServiceAll() {//measureType=PV, ServiceType=1(전체)
        if (_.contains($scope.searchStatContent, '1')) {//전체
            ...
            //'OCB 체크인', 'Syrup Push 팝업', 'Syrup Push Noti', 'Syrup 여기(버튼)'
            $scope.group2Headers.push({startColumnName: 'ocbCoinPv', numberOfColumns: 2, titleText: 'OCB 체크인'},
                ...
                {startColumnName: 'srpOkClkPv', numberOfColumns: 2, titleText: 'Syrup Push 팝업 확인'},
                {startColumnName: 'srpCnclClkPv', numberOfColumns: 2, titleText: 'Syrup Push 팝업 취소'},
    ...
    function getServiceSyrup() {
        if (_.contains($scope.searchStatContent, '1')) {//전체
            ...
            if (_.contains($scope.searchStatContent, '4')) {
                ...
                $scope.group2Headers.push({startColumnName: 'srpOkClkPv', numberOfColumns: 2, titleText: 'Syrup Push 팝업 확인'},
                    {startColumnName: 'srpCnclClkPv', numberOfColumns: 2, titleText: 'Syrup Push 팝업 취소'},
                    {startColumnName: 'srpIconBtnClkPv', numberOfColumns: 2, titleText: 'Noti 조회'},
                    {startColumnName: 'srpIconPv', numberOfColumns: 2, titleText: 'Syrup 여기(버튼)'});
            }
