...
/**
 * Created by lko on 2014-11-10.
 */
public abstract class AbstractDrtExcelView extends AbstractView {
    private static final String CONTENT_TYPE =
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";

    protected String fileName = "Excel_donwload.xlsx";
    ...
    protected final void renderMergedOutputModel(Map<String, Object> model,
        ...
        this.fileName = this.fileName.replaceAll(".xls|.xlsx", ".xlsx");
     ...
     * 리포트용 타이틀 정의.
     *
     * @param worksheet 엑셀 시트
     * @param menuName 서비스 메뉴 이름
     * @param headLength 헤더 길이
     */
     ...
     *
     * @param worksheet 엑셀 시트
     * @param headers Excel Header 정보
     */
    protected void buildHeaders(Sheet worksheet, String[][] headers, int pivotFlag, int headerColSpan)
            throws Exception {
     ...
     * @param worksheet 엑셀 시트
     * @param headers Excel Header 정보
     * @param headerColSpan 헤더 길이
     * @param headerCellStyle Excel 쎌 스타일
     */
    private void buildHeaderForPivot(Sheet worksheet, String[][] headers, int headerColSpan,
            CellStyle headerCellStyle) throws Exception {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            for (int j = 0; j < headersLength; j++) {
                ...
                if (i == 0 && StringUtils.isEmpty(headers[i][j]))
                    cell.setCellValue(headers[i][headerColSpan - 1]);
                else
     ...
     * @param headerCellStyle Excel 쎌 스타일
     */
    private void buildHeaderForSummary(Sheet worksheet, String[][] headers, int headerColSpan,
            CellStyle headerCellStyle) throws Exception {
        int headersLength = headers[0].length;
        for (int i = 0; i < 2; i++) {
            Row rowHeader = worksheet.createRow((short) (i + 2));
     ...
     * @param headerCellStyle Excel 쎌 스타일
     */
    private void buildHeaderForBoss(Sheet worksheet, String[][] headers, int headerColSpan,
            CellStyle headerCellStyle) throws Exception {
        int headersLength = headers[0].length;
     ...
     * @param headerCellStyle Excel 쎌 스타일
     */
    private void buildHeaderForDefault(Sheet worksheet, String[][] headers, CellStyle headerCellStyle)
            throws Exception {
        int headersLength = headers[0].length;
        Row rowHeader = worksheet.createRow((short) 2);
        rowHeader.setHeight((short) 500);
