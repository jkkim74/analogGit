...
/**
 * The VisitExceptPushOutlineExcelView class.
 * 
 * @author HoJin-Ha (mimul@wiseeco.com)
 * 
 */
public class VisitExceptPushOutlineExcelView extends AbstractExcelView {
	/**
	 * 방문개요 푸시제외 엑셀 저장.
	 * 
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
		...
		// 엑셀 바디 생성.
		List<ObsVstExcptPushSta> obsVstExcptPushStas = uncheckedCast(model.get("visitExceptPushOutlineData"));
		for (ObsVstExcptPushSta obsVstExcptPushSta : obsVstExcptPushStas) {
			// Create a new row
			Row row = worksheet.createRow(startRowIndex++);
			// Retrieve the 기준일자 value
			ExcelUtil.createCell(row, 0, Cell.CELL_TYPE_STRING, styleHVCenter, obsVstExcptPushSta.getStdDt());

			// Retrieve the UV value
			ExcelUtil.createCell(row, 1, Cell.CELL_TYPE_NUMERIC, styleHVRight, obsVstExcptPushSta.getUv().doubleValue());

			// Retrieve the LV(Login Visitor) value
			ExcelUtil.createCell(row, 2, Cell.CELL_TYPE_NUMERIC, styleHVRight, obsVstExcptPushSta.getLv().doubleValue());

			// Retrieve the RV(Returning Visitor) value
			ExcelUtil.createCell(row, 3, Cell.CELL_TYPE_NUMERIC, styleHVRight, obsVstExcptPushSta.getRv().doubleValue());

			// Retrieve the 방문횟수 value
			ExcelUtil.createCell(row, 4, Cell.CELL_TYPE_NUMERIC, styleHVRight, obsVstExcptPushSta.getVstCnt()
					.doubleValue());

			// Retrieve the 체류시간 value
			ExcelUtil.createCell(row, 5, Cell.CELL_TYPE_NUMERIC, styleHVRightComma, obsVstExcptPushSta.getTimeSptFVst()
					.doubleValue());

			// Retrieve the PV value
			ExcelUtil.createCell(row, 6, Cell.CELL_TYPE_NUMERIC, styleHVRight, obsVstExcptPushSta.getPv().doubleValue());

			// Retrieve the Bounce Rate value
			ExcelUtil.createCell(row, 7, Cell.CELL_TYPE_NUMERIC, styleHVRightComma, obsVstExcptPushSta.getBuncRate()
					.doubleValue());
		}
	}
}
