...
/**
 * The SyrupBleCellHandler(Syrup BLE 동적 헤더 처리) class.
 *
 * @author HoJin-Ha (mimul@wiseeco.com)
 *
 */
public class SyrupBleHeaderHandler implements BleHeaderHandler {
	@Override
	public MultiHeader buildHeader(String statContents) throws Exception {
		...
		if (StringUtils.indexOf(statContents, "1") > -1) {// 통계 항목 전체 선택
			multiHeader.setHeader1(ArrayUtil.addAll(header1, "BLE 수신항목", "BLE 수신항목", "BLE 수신항목", "BLE 수신항목", "BLE 수신항목", "BLE 수신항목",
					...
					"BLE 열람항목", "BLE 열람항목"));
			multiHeader.setHeader2(ArrayUtil.addAll(header2, "Syrup Push 팝업", "Syrup Push 팝업",
					"Syrup Push Noti", "Syrup Push Noti", "Syrup 여기(버튼)", "Syrup 여기(버튼)",
					"Syrup Push 팝업 확인", "Syrup Push 팝업 확인", "Syrup Push 팝업 취소", "Syrup Push 팝업 취소", "Noti 조회", "Noti 조회", "Syrup 여기(버튼)", "Syrup 여기(버튼)",
					"Syrup 체크인/URL 확인", "Syrup 체크인/URL 확인"));
			multiHeader.setHeader3(ArrayUtil.addAll(header3, "PV", "UV", "PV", "UV", "PV", "UV",
			...
			if (StringUtils.indexOf(statContents, "3") > -1) {
				multiHeader.setHeader1(ArrayUtil.addAll(multiHeader.getHeader1(), "BLE 수신항목", "BLE 수신항목", "BLE 수신항목", "BLE 수신항목",
						"BLE 수신항목", "BLE 수신항목"));
				multiHeader.setHeader2(ArrayUtil.addAll(multiHeader.getHeader2(), "Syrup Push 팝업", "Syrup Push 팝업",
						"Syrup Push Noti", "Syrup Push Noti", "Syrup 여기(버튼)", "Syrup 여기(버튼)"));
			...
			if (StringUtils.indexOf(statContents, "4") > -1) {
				multiHeader.setHeader1(
						ArrayUtil.addAll(multiHeader.getHeader1(), "BLE 클릭항목", "BLE 클릭항목", "BLE 클릭항목", "BLE 클릭항목",
								"BLE 클릭항목", "BLE 클릭항목", "BLE 클릭항목", "BLE 클릭항목"));
				multiHeader.setHeader2(ArrayUtil.addAll(multiHeader.getHeader2(), "Syrup Push 팝업 확인",
						"Syrup Push 팝업 확인", "Syrup Push 팝업 취소", "Syrup Push 팝업 취소", "Noti 조회", "Noti 조회", "Syrup 여기(버튼)", "Syrup 여기(버튼)"));
				multiHeader.setHeader3(ArrayUtil.addAll(multiHeader.getHeader3(), "PV", "UV", "PV", "UV", "PV", "UV", "PV", "UV"));
			}
			if (StringUtils.indexOf(statContents, "5") > -1) {
				multiHeader.setHeader1(ArrayUtil.addAll(multiHeader.getHeader1(), "BLE 열람항목", "BLE 열람항목"));
				multiHeader.setHeader2(
						ArrayUtil.addAll(multiHeader.getHeader2(), "Syrup 체크인/URL 확인", "Syrup 체크인/URL 확인"));
