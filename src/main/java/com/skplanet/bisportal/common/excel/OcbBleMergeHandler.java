...
/**
 * The OcbBleMergeHandler(OCB BLE 동적 헤더 병합 처리) class.
 *
 * @author HoJin-Ha (mimul@wiseeco.com)
 *
 */
public class OcbBleMergeHandler implements BleMergeHandler {
	@Override
	public void handleBleMerge(Sheet worksheet, MultiHeader headers, String statContents) throws Exception {
		...
		if (StringUtils.indexOf(statContents, "1") > -1) {// 통계 항목 전체 선택
			...
			worksheet.addMergedRegion(new CellRangeAddress(2, 2, 9, 10));
			worksheet.addMergedRegion(new CellRangeAddress(2, 2, 11, 14));
			...
			if (StringUtils.indexOf(statContents, "3") > -1) {
				...
				if (StringUtils.indexOf(statContents, "5") > -1) {
					worksheet.addMergedRegion(new CellRangeAddress(2, 2, 7, 10));
