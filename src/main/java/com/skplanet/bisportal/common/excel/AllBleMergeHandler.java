...
/**
 * The AllBleMergeHandler(전체 BLE 동적 헤더 병합 생성 처리) class.
 *
 * @author HoJin-Ha (mimul@wiseeco.com)
 *
 */
public class AllBleMergeHandler implements BleMergeHandler {
	@Override
	public void handleBleMerge(Sheet worksheet, MultiHeader headers, String statContents) throws Exception {
		...
		if (StringUtils.indexOf(statContents, "1") > -1) {// 통계 항목 전체 선택
			worksheet.addMergedRegion(new CellRangeAddress(2, 2, 5, 14));
			worksheet.addMergedRegion(new CellRangeAddress(2, 2, 15, 24));
			worksheet.addMergedRegion(new CellRangeAddress(2, 2, 25, 30));
			...
			worksheet.addMergedRegion(new CellRangeAddress(3, 3, 21, 22));
			worksheet.addMergedRegion(new CellRangeAddress(3, 3, 23, 24));

			worksheet.addMergedRegion(new CellRangeAddress(3, 3, 25, 26));
			worksheet.addMergedRegion(new CellRangeAddress(3, 3, 27, 28));
			worksheet.addMergedRegion(new CellRangeAddress(3, 3, 29, 30));
		} else {
			if (StringUtils.indexOf(statContents, "3") > -1) {
				...
				if (StringUtils.indexOf(statContents, "4") > -1) {
					...
					if (StringUtils.indexOf(statContents, "5") > -1) {
						worksheet.addMergedRegion(new CellRangeAddress(2, 2, 15, 20));
